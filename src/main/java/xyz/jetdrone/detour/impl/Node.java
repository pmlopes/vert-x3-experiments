...
import static xyz.jetdrone.detour.impl.Node.Type.*;
...
public class Node {
  ...
  Node() {
    this("", false, Node.Type.STATIC, 0, "", new ArrayList<>(), null, 0);
  }

  private Node(String path, boolean wildChild, Type type, int maxParams, String indices, List<Node> children, Handler[] handle, int priority) {
    this.path = path;
  ...
  private void insertChild(int numParams, String path, String fullPath, Handler[] handle) {
    Node n = this;
  ...
  public void addRoute(String path, Handler... handle) {
    Node n = this;
  ...
  public Handler[] search(final Context<?> ctx) {
    return search(ctx, true);
  }

  public Handler[] lookup(final Context<?> ctx) {
    return search(ctx, false);
  }

  private Handler[] search(final Context<?> ctx, boolean updateParams) {
