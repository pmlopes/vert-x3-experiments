...
gracefully degrades to pooling AJAX calls.

WebSockets bring a new level of interaction to the web, they really bring real time to web applications due to the fact
that its communication is bi-directional in contrast to the HTTP model where a client can initiate a data request to a 
server but not the other way around.

In this small example I will demonstrate how you can create a simple colaborative drawing app. The idea is simple, all
...
If you followed the previous series on Vert.x development, you saw that Java and Maven were the main topic, since Vert.x
is polyglot I will focus on JavaScript and NPM as my programming language and package management tool.

With NPM we start by creating a package.json, in order to do this we should run:

```
npm init
```

This will present a selection of questions and in the end you should have a skeleton `package.json` file. This skeleton
is very basic so we need to add a dependency to Vert.x so we can run the application. We can add it to the 
`dependencies` property and it should look more or less like this:
...
If you do not know why there is the dependency on `vertx3-full` please check the older blog post about it.

## Project Structure

This post has no preference over project structure, so if you do not agree with the structure used here feel free to
use what you feel best. For this example we will keep it to:
...
we start by creating a bridge using the `vertx.EventBus` object and we define where we connect to by using the details
of the current window location.

We then add a `onopen` listener that will subscribe to the address `draw` on the event bus so we can listen to all
messages regarding drawing and perform the drawing actions. Since listening is not enough we also add a mouse listener
to the document and when it moves we publish events to the `draw` address.

Note that we are using publish and not send, the reason should be obvious, we want everyone to know this users mouse
movements, we are not interested on sending the events to just a single user. You can see now that if you want to have
